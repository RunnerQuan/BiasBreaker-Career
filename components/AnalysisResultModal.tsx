                  ...
                  <div className="priority-list">
                    {result.findings.slice(0, 3).map((finding, index) => (
                      <article key={`${finding.type}-priority-${index}`}>
                        <span>{index + 1}</span>
                        <div><strong>{findingTypeLabel(finding.type)}</strong><p>{finding.suggestion}</p></div>
                      </article>
                    ))}
                  </div>
                </section>
                  ...
                  <div className="finding-list">
                    {result.findings.map((finding, index) => (
                      <article key={`${finding.type}-${index}`}>
                        <div>
                          <strong>{findingTypeLabel(finding.type)}</strong>
...
function findingTypeLabel(type: string) {
  const direct = FINDING_TYPE_LABELS[type];
  if (direct) return direct;

  const normalizedKey = type.replace(/[\s_-]+/g, "").toLowerCase();
  ...
  return /[A-Za-z]/.test(type) && !/[\u4e00-\u9fff]/.test(type) ? "待优化问题" : type;
}
