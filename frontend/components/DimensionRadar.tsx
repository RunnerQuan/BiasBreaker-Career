    ...
    <div className="report-radar-card" aria-label="四项维度评分雷达图">
      <svg viewBox="0 0 220 220" role="img">
        <g fill="none" stroke="#cbd5e1" strokeWidth="1.2">
          <path d="M110 24 L196 110 L110 196 L24 110 Z" />
          <path d="M110 50 L170 110 L110 170 L50 110 Z" />
          <path d="M110 76 L144 110 L110 144 L76 110 Z" />
          <path d="M110 24 V196 M24 110 H196" strokeDasharray="3 5" />
        </g>
        ...
        <RadarLabel x={110} y={8} anchor="middle" label={displayLabels.keywordCoverage} score={scores[0]} />
        <RadarLabel x={208} y={98} anchor="middle" label={displayLabels.structureClarity} score={scores[1]} />
        <RadarLabel x={110} y={210} anchor="middle" label={displayLabels.evidenceStrength} score={scores[2]} />
        <RadarLabel x={12} y={98} anchor="middle" label={displayLabels.atsReadability} score={scores[3]} />
      </svg>
    </div>
  );
}
...
function RadarLabel({ x, y, anchor, label, score }: { x: number; y: number; anchor: "middle"; label: string; score: number }) {
  return (
    <>
      ...
      <text x={x} y={y + 18} textAnchor={anchor} className="report-radar-value">
        {score}
      </text>
    </>
  );
